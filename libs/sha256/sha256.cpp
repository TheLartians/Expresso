...
void SHA256::final(sha256_hash & digest){
  final(digest.data());
  
  std::hash<sha256_hash::value_type> hasher;
  
  digest.quick_hash = 0;
  
  for (size_t i = 0; i < digest.size(); ++i){
    digest.quick_hash = digest.quick_hash * 31 + hasher(digest[i]);
  }
...
std::string sha256_hash::hex_string()const{
  std::string buf;
